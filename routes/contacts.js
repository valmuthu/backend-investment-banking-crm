...
router.post('/', contactValidation.create, handleValidationErrors, async (req, res) => {
  try {
    ...
    const existingContact = await Contact.findOne({
      userId: req.user.id,
      name: { $regex: new RegExp(`^${req.body.name}const express = require('express');
const { Contact } = require('../models');
const { authenticateToken } = require('../middleware/auth');
const { contactValidation, paginationValidation, handleValidationErrors } = require('../utils/validators');
...
, 'i') },
      firm: { $regex: new RegExp(`^${req.body.firm}const express = require('express');
const { Contact } = require('../models');
const { authenticateToken } = require('../middleware/auth');
const { contactValidation, paginationValidation, handleValidationErrors } = require('../utils/validators');
      ...
      try {
        ...
        if (skipDuplicates) {
          const existing = await Contact.findOne({
            userId,
            name: { $regex: new RegExp(`^${contactData.name}const express = require('express');
const { Contact } = require('../models');
const { authenticateToken } = require('../middleware/auth');
const { contactValidation, paginationValidation, handleValidationErrors } = require('../utils/validators');
...
, 'i') },
            firm: { $regex: new RegExp(`^${contactData.firm}const express = require('express');
const { Contact } = require('../models');
const { authenticateToken } = require('../middleware/auth');
const { contactValidation, paginationValidation, handleValidationErrors } = require('../utils/validators');
